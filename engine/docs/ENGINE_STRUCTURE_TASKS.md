...
| S4 | L2 projections + `node_id` + structure map | B | **D18 schema-born fixture (S4.5)** | `WIP` (S4.0–S4.5 `DONE`, **gate green — schema v1 born**; S4.6-pre freeze + S4.6a sidecar `DONE` (#31/#32 closed; post-audit remediation applied 2026-07-02 — digest split + typed gate error + engine writer); S4.6b authoring-loop tooling `DONE` 2026-07-03 (#34 — gate/validate/status/stamp/explain + DT-4 payload witnesses + PLL seeder; draft seeded, worklist live); S4.6c structural-contents observations `DONE` 2026-07-19 (#90 — source-locked advisory facts before authoring); S4.6d visual review bridge `WIP` 2026-07-19 (#93 with Workbench #2 — implementation and default suites green, awaiting review/merge); S4.7 scale + rebind remediation `DONE` 2026-07-19 under #33/#48/#84 and epic #85 closeout) | `TODO` (S4.6 map — **unblocked, tooling + reviewed source observations live**) |
...
| S1.4 | **Production-stream round-trip GATE (real-input floor):** reconstruct each of `copy{1,2,3}_raw` byte-for-byte **through the atom-store's public read path consumers use** (witness iteration, filter-by-witness, canonical-projection load — exercise the shapes S10.2's consumers call; never a private helper, never re-reading raw source). Assert the **capture-completeness invariant** (every source byte is inside an atom's `raw_span` or recorded as declared inter-atom/furniture) **and span topology** (ordered, non-overlapping coverage with explicit gap records). **Core landed** (`structure/roundtrip_gate.py`, #21): `GapRecord` (explicit declared gap), `gap_records` (the tiling/topology walk — `assert_capture_tiles` now **delegates** here, single owner), `reconstruct_source` (whole-artifact byte-exact from stored atom+gap text — the gate's `==source` compare catches an `atom.text` drifted off its span, a drift the per-atom hash re-slices past), `assert_no_wholesale_exclusion` (owns the S1.3b-deferred "excluded everything" seam), `assert_production_roundtrip` (the gate entry). Runs over real `copy{1,2,3}_raw` (whole-artifact reconstruct + overlap/implicit-gap/text-drift/all-excluded negatives). **5-lens overlapping adversarial audit** (spec/correctness/test-quality/neutrality/architecture) → no blockers, all mutants killed (2 survivors proven *equivalent*); hardening applied: `GapRecord` whitespace-only enforced at construction (durable record fails loud on content), `reconstruct_source` rejects an out-of-order atom stream (no silent re-sort), canonical-stream out-of-whole-artifact-scope tripwire + the persist-gaps decision recorded for S1.5 (carried note), 0.5-floor over-claim softened. 32 tests, 14 mutants killed. **Closure LANDED (post-S1.5, #21):** `test_roundtrip_gate_closure.py` routes the gate through the S1.5 store's **public read path** — each witness captured → `save_stream` → `load_stream` → reconstructed byte-exact from the **loaded** atoms+gaps, checked against the independently-read fixture (non-tautological — the test brings the oracle, not the store's own `source_hash`); **witness iteration** via a new `stream_ids(workspace)` enumeration primitive (sorted `data/atoms/*.json` stems) + filter-by-`.kind`; **canonical load** routed through `assert_reference_integrity`, never the whole-artifact gate (the carried scope decision); full-gate `assert_no_wholesale_exclusion` over the loaded stream. **Back-door-read negative:** corrupt the **store** on disk while the raw witness fixture stays byte-identical → `load_stream` fails loud (it validates the *store's* bytes, not the still-correct raw file a back-door would have read + falsely passed); + overlap / implicit-gap negatives through the same read path. 7 closure tests; closure binding mutation-proven (`stream_ids` glob + that the negatives genuinely depend on `load_stream`'s self-checks); full suite **520 green**. | §3.0, §9; D22 | S1.3a | round-trip + negative tiers: byte-exact via the public read path; overlap/implicit-gap/back-door read each fail loud | `GATE` | `DONE` |
| S1.5 | **Atom-store persisted schema + independent version** (per-witness + canonical streams), its own lineage entry + **its own S8.1 stale class** (M3); store round-trip asserts **serialization-invariance** of `raw_span` / `raw_source_hash` / `geom` / `derived_from` (not just text); reference-integrity (every `derived_from` back-link resolves). **Core landed** (`structure/atom_store.py`, #22): `AtomStream` (`witness`\|`canonical` — a witness tiles one source so it carries gaps + a `source_hash` anchor; canonical tiles no single source so it carries neither, integrity = reference-integrity + the per-atom S1.2 floor), `to_json`/`from_json` (full-fidelity envelope; `from_json` is a **total load-boundary contract** — a valid stream or `StaleArtifactError`, errors.py exit **10**, the new M3/S8.1 stale-class raiser; a stale `schema_version`, wrong `stale_class`, missing key, unknown kind, or a wrapped model-invariant each fail loud), `save_stream`/`load_stream` (atomic JSON under `data/atoms/<id>.json`; **load runs the anchored round-trip self-check** `hash_raw(reconstruct_source(atoms, gaps)) == source_hash` → a persisted `atom.text`/gap drifted off its span fails loud, the whole-artifact #21 value now anchored in the store), `assert_reference_integrity` (every canonical `derived_from` resolves, else `CaptureError`). **Persist-gaps decision applied** (carried #21 audit): the per-witness `GapRecord` stream is persisted as **bytes** (a mixed-whitespace `\n \t\n` gap proves bytes-not-width — atoms-only would round-trip vacuously on PLL's newline-only copies); `source_hash` anchors the round-trip non-vacuously. `ATOM_STORE_STALE_CLASS="atom-stream"` registered alongside `ATOM_STORE_SCHEMA_VERSION=1` (artifacts.py). **5-lens overlapping adversarial audit** (cold-read / data-loss-falsifier / premise-demolisher / downstream-delivery-skeptic / taxonomy-atomicity — deliberately divergent *stances + briefings* to break confirm-shaped patterning) → real defects found + fixed: (D1) `from_json`/`load_stream` "total contract" leaked raw `TypeError` (wrong-typed field) / `json.JSONDecodeError` (non-JSON file) / non-object document — now a `Mapping` gate + `(ValueError, TypeError)` wrap + JSON-decode wrap, all → `StaleArtifactError`; (D2) the persisted per-atom `raw_source_hash` was **inert** at load (whole-artifact anchor checks the concatenation → a *compensated* drift across an atom boundary false-passed, and canonical streams had **no** load text-integrity) — added `assert_atom_hashes` (per-atom `hash_raw(text)==raw_source_hash`, runs on every load incl. canonical); (D3) `atom_id` uniqueness unenforced despite `atoms.py` naming the store the enforcer — now `duplicate_atom_ids` in `__post_init__`; (D4) NaN/inf geom float broke `==` + wrote non-RFC-JSON — `allow_nan=False` in `jsonio` (fail-loud at save); (J3) read-path `stream_id` not flat-stem-guarded — `_check_stream_id` now in `stream_path`; (J5) canonical atom with empty `derived_from` passed vacuously — reference-integrity now requires ≥1; (J6) invented-coords-on-absent geom silently normalized — `_geom_from_json` reconstructs faithfully + lets the model guard fire; (J2) `CaptureError` docstring broadened to own cross-stream reference-integrity. 64 store tests (synthetic + real `copy{1,2,3}`), **29 atom_store mutants + the jsonio guard killed, 0 survivors**; full suite **513 green**, ruff clean, neutrality 48. **Unblocks the S1.4 (#21) closure** (route #21 reconstruction through `load_stream` + the back-door-read negative). | §3.5, §3.6, §11.1; D21 | S1.3a | reference-integrity + round-trip tiers: schema validates, version registered, serialization-invariant, back-links resolve | `BUILD` | `DONE` |
| S1.6 | **Stream QA — duplicated-section twins (flagger, never deduplicator):** detect near-identical spans in the frozen streams via three escalating signals (S-1 text twins above a length floor / S-2 per-atom provenance signature: cross-witness-complementary vs within-witness vs both-at-both / S-3 geometry physical-instance counting off S2.1's box-consumption), classify with calibrated abstention (`unclassified`, never a guessed class), **verdicts human-only** — no band ever auto-verdicts; long intentional repeats flag by design and receive human `intentional` verdicts. Three-artifact split: regenerable `work/data/stream_qa/twin_report.json` (stream-fingerprinted; stale = consumers fail loud, absent = seeder notes and proceeds) / tracked `books/<id>/review/twin_verdicts.json` (provenance-carrying human verdicts; `accidental_merged` verdicts carry the approved word-level merged text — materialization into a superseding stream revision defers to S8.1) / applied `twin-of` relations in the L3 store (ratified-truth only, `pending` never enters). Slices: **A** detector+report+worklist (deps met; acceptance: finds the P1-ch22 canonical alignment twin `canonical_01817`/`01979`, silent on running heads/folios/the end-matter index) → **B** geometry adjudicator (gated on S2.1 #37's sidecar) → **C** verdict CLI `python -m engine.structure.twins` (own entry; shared review front-door deferred to #40's close) + DT-4 merge + DT-5 seeder consumption (pending annotates, ruled binds; strict twins→seed direction). Origin evidence: PLL #655 (19K chars, 0.977) + the P1-ch22 canonical twin (2026-07-04) + the seeder's 22-flag report. | `docs/twins_plan.md` (**RATIFIED 2026-07-04**, DT-1…DT-6 + slice-C/tracker rulings; numeric ledger P-1…P-4 measured on PLL streams and ruled at slice A — gates row-DONE, not build); GitHub issues minted at build-start (parent + one per slice) | S1.4, S1.5 (slice B: S2.1 #37; slice C merge materialization feeds S8.1) | slice A acceptance run green on ruled P-1…P-4; slices B/C per plan; DT-6 ledger ruled | `BUILD` | `TODO` |
...
> **S1.5 audit — principled deferrals (not gaps).** Two findings from the 5-lens audit were
> *deferred by decision*, recorded so they are not silently dropped:
> - **Stale-vs-corrupt error split → decide at S8.1.** `from_json` funnels both a genuinely *stale*
>   `schema_version` (migratable — the M3/S8.1 stale-class hook) and a *corrupt/malformed* envelope
>   (un-migratable — re-capture) into one `StaleArtifactError` (exit 10). The message distinguishes
>   them; the *type/code* does not. Harmless until a consumer must branch "auto-migrate vs alert" — and
>   that consumer is **S8.1's stale-class migration router**, which does not exist yet. Deciding the
>   split (sibling `MalformedArtifactError`, or a structured discriminator attr) now would be YAGNI
>   against an unbuilt requirement; revisit when S8.1's router is designed ([[deferral_for_information]]).
> - **`Atom` lacks a `len(text)==raw_span`-width invariant (asymmetry with `GapRecord`, which has it).**
>   This is the structural enabler of the *compensated-drift* false-pass; S1.5 closed the **store's**
>   exposure at load via `assert_atom_hashes` (per-atom `hash_raw(text)==raw_source_hash`), but the
>   model-level invariant is an **S1.1/atoms.py** surface (frozen/`DONE`), out of S1.5 scope. Adding it
>   to `Atom.__post_init__` is defense-in-depth for any future non-store caller — a candidate burnish
>   for an S1.1 follow-up, not reopened here.
...
| ID | Deliverable | Refs | Deps | Done-when (tier) | Tag | St |
|---|---|---|---|---|---|---|
| S2.0 | **bbox + text↔box-alignment PROBE (GATE).** On real PLL pages, measure (i) does a usable OCR word-box layer exist, and (ii) **can those boxes be trusted as anchors for the witness text** — the boxes come from a *different* OCR pass (PyMuPDF) than the witness text (IA-Tesseract / Gemini), so "carries its primary witness's box" is itself a cross-engine match (M1). Sample **stratified across** front matter, dense prose, page furniture, chapter starts, footnotes, bad-OCR regions, and the embedded-letter area (≥10 pages is a floor, not the spec). A negative/low-quality outcome **(a)** reroutes to **S2.1-alt** and **(b)** demotes geometry in **S5** (primary → tie-break / no-geometry, M2) | §3.0; D30; §1 portability note (`pll` PDF is an image scan) | S0.1 | a written probe result over the strata; both the S2.1 path AND the S5 geometry-mode are **chosen by evidence**, with measured text↔box alignment quality | `GATE` | `DONE` |
| | **S2.0 outcome (POSITIVE, conditional — revised post 5-lens audit, `docs/probes/s2_0_adversarial_audit.md`).** Probe `books/per_la_liberta/probes/s2_0_geometry_probe.py`; written result `docs/probes/s2_0_geometry_alignment.md`. Both PDFs are image scans (0 native boxes); box layer **generated** by fresh PyMuPDF+Tesseract(ita,300dpi). Token anchorability vs copy3/Gemini (n=37): BoW median **0.939**/mean 0.925; **content-token** BoW median **0.929** (what re-bind gets). Body is two-column (detector finds **30/37**, not the audit-killed "37/37" artifact) → naive full-width ordered **0.49**, column-order recovered from box coords lifts it to **mean 0.92 / 87% pass@0.85 on two-column pages (n=30)** but **mean 0.851 / 73% pass over all pages** (single-col + hard edges drag it). copy1 (structural witness) confirmed column-ordered (0.98). **Decision (a): S2.1 NORMAL path** (not S2.1-alt) + segmentation front-end (density / column-detect / human-review; see S2.1 row). **Decision (b): S5 geometry = conditional-primary, RE-GATE after S2.1** (NOT settled-PRIMARY — the audit showed the pre-registered *naive* metric is 0.49/no-geometry-band and PRIMARY rested on a median hiding mean 0.82 + a detector prototype); primary-grade on confident two-column body pages, re-gated on mean + per-page pass-rate after S2.1 builds the real detector + two-branch order sourcing; absence first-class, coords never invented | | #18 | written probe result delivered; both decisions evidence-chosen + adversarially audited | `GATE` | `DONE` |
| S2.1 | `GeometrySource` seam (Protocol) + backend on the **path S2.0 selected** (= NORMAL, fresh PyMuPDF+Tesseract(ita) OCR — both PDFs are image scans, no native layer): textpage/bbox layer → `atom.geom` word-box union, via an **explicit witness-text↔geometry matcher with its own fail-loud mode**; the matcher writes `{geometry_engine, matched_witness_id, match_method, match_confidence}` into S1.1's frozen slot (all four provenance fields — the earlier three-field prose under-quoted the frozen `Geom` contract; corrected 2026-07-03, s2_1_plan audit R21); **unmatched boxes are unusable for primary re-bind**; canonical atom carries its primary witness's box **only where matched**. **S2.0-mandated segmentation front-end** (design inputs in `docs/probes/s2_0_geometry_alignment.md` §"S2.1 design inputs"; revised post-audit): **(0) two-branch reading-order sourcing** — when a column-correct text witness exists (PLL copy1/2/3, copy1 verified 0.98) it is the order ORACLE and boxes position-match it (geometric detector = cross-check); when NO prior witness exists (image-only — must not be presumed away) the geometric detector is the primary order source + human-in-loop backstop; **(1) density pre-check** — a **two-sided / band classifier** (NOT a single fixed ink threshold — audit proved ink-fraction is a non-monotone continuum: dark endpapers read 0.97, content chapter-ends read <0.038) flagging near-blank pages so hallucinated boxes (p6: 658 boxes / 7 tokens) aren't trusted; **(2) column detection** via projection-profile / Recursive X-Y Cut valley (the discriminator; mirror-symmetry rule DROPPED as redundant + wrong on asymmetric multi-column; cross-page prior retained) — yields reading order for free; **(3) human-review worklist** with a SPECIFIED confidence metric/threshold/human-verdict-schema/volume-bound (audit: not a slogan) for pages the front-end can't score, **before** the gate — classifiers calibrate to **abstain** (per [[project-ingestion-human-in-loop]], S5.2 monotone-strictness). Page-level `geom.present=false` only after BOTH auto-detection and human review decline; atom-level absence on an accepted page is automatic-with-recorded-reason (s2_1_plan R10/R19) | §3.0, §11.1; D30; `docs/probes/s2_0_geometry_alignment.md`; plan `docs/s2_1_plan.md` (**RATIFIED 2026-07-03**; DT-1…DT-13, red-first G-1…G-26; DT-13 = the copy2-only eligibility gate, still-open by design, DoD-gating for row-DONE only; §8 numeric proposals P-1…P-6 all RULED 2026-07-03 — P-5 = the two-leg auto-absent tripwire, red row G-26) | S1.1, S2.0; issue #29 (parent; children #35–#40 minted 2026-07-03, two slices: #35–#37 seam/backend/matcher, #38–#40 front-end) | seam injectable; backend yields matched boxes (+ provenance) for a PLL page fixture; unmatched → `geom.present=false`, not invented; low-confidence pages routed to human review | `BUILD` | `DONE` |
| S2.1-alt | **Negative-branch fallback as a real deliverable** (only built if S2.0 is negative/below-threshold): name the coordinate source (word-coordinate re-derivation, or accept no-geometry), the atom-matching method, the acceptable quality threshold for S3/S5, and **what is disabled if unmet** (S3's geometry base layer; geometry as S5's *primary* anchor → S5 runs geometry-demoted / no-geometry). Not an architectural pause — a scoped path. **S2.0 outcome: NOT triggered** (probe positive — geometry viable, S5 conditional-primary); spec retained as the conditional fallback | §3.0; D30 | S2.0 (negative) | the fallback path is specified + (if triggered) implemented; S3/S5 know exactly what they lose | `BUILD` (conditional) | `N/A (S2.0 positive)` |
> **Pointer (S2.1.4 / #38 close, 2026-07-06):** the density band pre-check (`structure/segmentation.py`)
> is built + calibrated. **DT-6 amended (RULED by Ben 2026-07-06 at the calibration checkpoint):** the
> classifier is now **five-class** — `cover` added (position × saturation: a near-saturated leaf at the
> document extremes is binding material, auto-declinable) and content is **ink-gated** (`ink < ink_dark_min`
> checked before content, so a solid-dark page is never mislabelled content on hallucinated yield — calibration
> page 278, the back cover: ink 1.0 / 1189 phantom boxes / 0.753 yield). The classifier takes page position;
> two new required book-config bands (`cover_edge_leaves`, `ink_saturation_min`). **Band values RATIFIED**
> (`books/per_la_liberta/manifest.json` `segmentation.density_bands`: yield 0.70 / boxes 40 / blank 0.15 /
> dark 0.60 / margin 0.05 / cover_edge 7 / saturation 0.90); **labelled ground-truth set** (3 cover / 27 content
> / 12 near_blank over 43 pages) in the tracked `review/density_calibration.json`, reproduced by
> `probes/s2_1_density_probe.py`. Manifest schema gained an optional `segmentation` property (backward-compatible;
...
> pins detector-recovered reading order at `ordered_coverage == 1.0`. Column-decision policy
> **RATIFIED by Ben 2026-07-06** — `decision_threshold=0.50` / `hysteresis_margin=0.15` (evidence in
> `docs/probes/s2_1_column_report.md`: col2_score sharply bimodal — 59 pages ≈0.0, 211 ≥0.80, empty
> [0.05,0.40) valley, 5 pages in-margin — via `probes/s2_1_column_probe.py`); now in `manifest.json`
> `segmentation.column_detector`, loader builds the `ColumnDetector` via `from_config`. Red-first +
> mutation hunt **32/32**; three adversarial audits (wide + narrow found & drove the shadowing fix +
> a Rule-A delta re-audit). **DT-7 amendment (Ben 2026-07-06):** #40's run-report tooling will
> auto-*propose* these per-book from the col2_score distribution (antimode/valley, abstain on
> non-bimodal), human-ratified + frozen to config. Row stays `WIP` — #40 (worklist + verdict CLI +
> book-wide `order_qa` feed) remains.

> **Pointer (S2.1.6 / #40 close, 2026-07-06):** the human-review worklist + verdict CLI + volume
> bound + the DT-12 `order_qa` feed + the DT-7 auto-propose are built and run book-wide — **all six
> S2.1 children (#35–#40) closed, so the S2.1 row goes `DONE`.** New engine modules
> `structure/geom_review.py` (worklist candidates + `input_fingerprint` + per-stage volume bound
> **G-13** + verdict application/CLI **G-14/G-22** [idempotent projection, stale-refuse-and-retain,
> unknown-action fail-loud] + on-demand PNG overlays + `page_order_qa` + CLI `python -m
> engine.structure.geom_review --book <id> {status|apply|record}`) and
> `structure/column_calibration.py` (`propose_column_policy`: valley/antimode, calibrate-to-abstain).
> `geom_sidecar` **schema v2** adds the matched-page detector fields `n_cols`/`n_cols_source`/`order_qa`
> + `with_detector_fields` (the S2.2 feed). Artifact split (DT-10): generated
> `work/state/geometry_review/worklist_candidates.json` + on-demand
> `work/output/geometry_review/overlays/page_NNNN.png`; tracked human verdicts
> `review/geometry_verdicts.json`. Volume bound `review_fraction_max` (default 0.15, book-tunable in
> `manifest.segmentation`). **PLL run** (`s2_1_run_stats.json`): 25 worklist candidates {locate 7,
...
| ID | Deliverable | Refs | Deps | Done-when (tier) | Tag | St |
|---|---|---|---|---|---|---|
| S4.0 | **Constants + error-code module** (moved out of S4.4 — X18; decision-owned by D-S4-F): `STRUCTURE_MAP_STALE_CLASS`; pre-placed **inert** `RELATION_STORE_STALE_CLASS` (O1 — declares a future layer identity, no relation loader exported); the `schema_status` module map (§1.2.2); the closed **`EC.*` code set** in `structure/errors.py` (§4.0) so B-2+ can assert on codes | s4_plan §1.1/§4.0; D-S4-F | S0 | inv 12a (constants + inertness), 12c (closed-set pin + partition), 15 (neutrality) | `BUILD` | `DONE` |
| S4.1 | L2 projection model: container vs leaf; open per-book block vocabulary (PLL: `paragraph`, `verse`, embedded-letter container); **no-double-ownership** invariant; **B re-groups / re-types at the node level** over existing, unchanged atom ids (the block-classifier output is correctable, R3/D5; **true re-atomization — split/merge → new atom ids — is S8.2/D25**, s4_plan §1.3.5) | §3.1, §3.2, §11.2; D10, **D5/R3** | S1.1 | property: no atom owned twice; ragged depth 0–4 + recursion + **heterogeneous sibling classes** representable; a mis-typed atom is corrected by B (re-type) | `BUILD` | `DONE` |
| S4.2 | `node_id` identity + minting split: opaque, persisted, **never recomputed** from position/designation/content; humans mint containers, extractor machine-mints leaves (counter/ULID); `minted_by` field | §3.4; D11, D20, D33 | S4.1 | property: id stable across re-serialize **and after a positional move** (re-mint proves position-independence) | `BUILD` | `DONE` |
| S4.3 | Handle policy + rendered handles + alias records: per-node-class `handle_policy` (`position-path` \| `designation-string` \| `title` \| …) with inheritance; `(node_id, handle_policy)` renders `short`/`parse_md`/`html_slug`; alias record `{handle_type,value,scope,locale_or_witness,target_node_id,valid_from,valid_to,status}` | §3.4, §3.6; D12, D24, D33 | S4.2 | property: handle change leaves `node_id` fixed, old handle survives as alias | `BUILD` | `DONE` |
| S4.4 | `structure_map.json` **schema** + **lineage manifest** header. Manifest carries, **each independently versioned with its own stale class** (M3): raw witness hashes; the **atom-store** schema version (S1.5); the **relation-store** schema version (S7.1c); `canonical_stream_id` + canonical-projection hash; the **resource + normalizer** versions (S3.0); profile + recognizer versions. `geom` shape consumed from S1.1's frozen slot (the frozen field set incl. match-provenance; the `canonical_geometry_hash` covers the REGION fields only — `{atom_id, present, page, bbox}`, provenance deliberately excluded, s4_plan §3.E.1) | §3.5, §3.6, §11.2; D21 | S4.1–S4.3, **S1.1 (geom shape)** | schema validates; reference-integrity tier (every ref resolves); manifest lists all three persisted-layer schema versions; schema's `schema_version` `const` **equals** `STRUCTURE_MAP_SCHEMA_VERSION` — bound by validating the version-derived fixture (`_generate_structure_fixture.py`) against the schema, **not** a hand-copied literal (the `manifest.schema.json` precedent hard-codes the const with no Python constant behind it — do not repeat it) | `BUILD` | `DONE` |
| S4.5 | **D18 GATE (schema born) — second-structure synthetic fixture built to DIFFER from PLL:** depth-0 body (no parts), `designation-string` handle policy, **non-ordinal headings**, **mismatched body segmentation**, alias-uniqueness-in-scope, **malformed/incomplete-manifest rejection** (a negative fixture — a stored-vs-live staleness *comparison* is S8.1, M9), **relation-endpoint shape / reference placeholders** (*resolution* is S7/S9.4, R2-03). **Folded into the S4 keystone (O5): this gate going green is what closes the milestone** — flipped `schema_status` v1 → `born` at B-6. **This is the SAME adversarial fixture S9.4 drives end-to-end** (reuse, not a new one). **Distinct from `books/synthetic`** (prefazione + 1 part, Italian ordinals, H2/H3 — a miniature PLL, *not* a differ-fixture) | §6, §9; D18; `feedback_single_fixture_blind_spots` | S4.1–S4.4 | the hand-authored fixture validates; **schema is not "born" until it does**. (Generality is proven separately at S9.4) | `GATE` | `DONE` |
| S4.6-pre | **Freeze + persist the PLL atom streams** (row added 2026-07-02 from runway.md §3 item 1): canonical atom ids are **positional** (`canonical_NNNNN` from capture) — a capture-algorithm change renumbers the stream, and S4.6's hand-authored map references those ids *before* S5's re-bind machinery exists. Mitigation is procedural: capture the real copies through the live producers, persist via the S1.5 store into the PLL book workspace, record the stream hashes (the manifest's stream hashes turn later drift into S8.1-detectable staleness, not silent corruption). Cheap; **must precede S4.6** | runway.md §3.1 | S1.3a, S1.5 (#22); issue #31 | PLL witness + canonical streams persisted in the book workspace; hashes recorded; S4.6 authoring references only these frozen ids | `BUILD` | `DONE` |
| S4.6a | **Authoring-evidence sidecar — engine half** (runs BEFORE S4.6 despite the id). **Owner: engine** (vs S4.6's Ben/HITL): `structure/evidence.py` — loader for `structure/schema/authoring_evidence.schema.json` + the digest-staleness validator (**amended by the 2026-07-02 post-audit remediation, user-ratified — see s4_plan §1.4.1b:** the staleness key is SPLIT into `decision_digest` (`node_class` + ordered `children` node ids; never machine-refreshed) and `extent_digest` (slot-aware since the F5/Option B closure 2026-07-02: own per-slot binding + flat beneath-union; S5-re-stampable), each its own gate finding kind; `map_revision` is bookkeeping, never the trigger. Same remediation: `EvidenceGateError` exit 12 + non-raising `evidence_findings` producer; `book` + `stale_class` persisted in the document; deny-by-default `write_authoring_evidence` + `build_evidence_entry` stamper; load-boundary OSError/RecursionError hardening here and in `load_structure_map`/`load_freeze_record`). Sidecar optional at generic load, required at the S4.6 authored-map gate (every `minted_by:human` container ↔ one non-stale evidence entry). Own `AUTHORING_EVIDENCE_SCHEMA_VERSION` + stale class (M3). Inherits inv 15 + ships a digest-staleness red test. **PLUS store-backed reader glue (folded 2026-07-02, runway.md §3 item 2):** workspace → `load_stream`× → `StreamAtomReader`, composing `assert_reference_integrity` on load — `StreamAtomReader` takes in-memory streams, and nothing yet loads a workspace's persisted streams into a reader; both the S4.6 authoring gate and S5's re-bind consume it | s4_plan §1.4.1a/b/c; Audit 15, M9/X20; runway.md §3.2 | S4.4, S1.5 (#22); issue #32 | digest-staleness red test; inv 15; the S4.6 gate consumes it; the glue loads a workspace's persisted streams into a `StreamAtomReader` with reference integrity asserted | `BUILD` | `DONE` |
| S4.6b | **Authoring-loop tooling** (row added 2026-07-03 from runway.md §3 item 3; plan `s4_6_tooling_plan.md` RATIFIED 2026-07-02, DT-1..DT-8). **Owner: engine.** `structure/authoring/` package: **composite gate** `assert_authoring_integrity` (substrate-first: freeze pin → streams → pin↔live → map Tier-1+2 → evidence; S8.1's stored-manifest-vs-live comparison deliberately NOT here, pinned by a passing test), non-raising `validate_authoring` + `--watch` mtime poll, worklist `authoring_status`/`render_status` (kinds as columns, node attribution from THE findings core — never message-parsing), single-node `stamp_evidence` (read-modify-write; **no bulk path, permanently** — DT-6), digest-diff explainer `explain_evidence_drift`. Explainer needs no baseline: **DT-4 payload witnesses** (user-ratified) — each sidecar entry persists its digest payloads, run-length-encoded, **self-verified at load** (incoherent = `StaleArtifactError` naming the node; a forged digest cannot load); §1.4.1b + the sidecar schema amended same commit, inside the still-open v1 free-edit window. Book side: `books/per_la_liberta/seed_structure_map.py` — candidate seeder (exact-first + guarded-fuzzy heading assignment, abstain-and-flag; the fuzzy guard exists because one absent heading otherwise stole ch32's atom and cascaded ch24–33 into abstention). **Demo run:** 120 nodes / 61 containers / 56 chapters seeded, 4,786 atoms fully covered, validates clean, 22 review flags, 1 honest abstention (P2 ch22 heading absent from the stream); gate reds all-`missing` (exit 12) = the live S4.6 worklist | s4_6_tooling_plan §2–§7; runway.md §3.3 | S4.6a (#32), S4.6-pre (#31); issue #34 | plan §4's invariant rows red-first; mutation hunt 36/36 killed (incl. the E12/E13 coherence-masking and A4 footer-masking lessons, fixed by sharpening tests); live demo: seed → status 61-missing → gate exit 12 → validate catches a planted `DANGLING_REF` | `BUILD` | `DONE` |
| S4.6c | **Generalized structural-contents source observations before authoring.** **Owner: engine.** The source-locked, advisory observer searches declared structural labels through generic DjVu XML and plain-text adapters, records every exact normalized sighting with a stable source locator, and emits per-source zero/count summaries plus a deliberately weak `contents-like` page feature. Its strict schema/loader and deny-by-default writer fail loudly on missing, malformed, or stale inputs. It imports no book/language profile, assigns no scalar confidence, does not assert contents-vs-topical-index authority, and cannot mutate `structure_map.json` or `authoring_evidence.json`; those semantic policies remain S9.1/#83 and #20. **PLL diagnostic:** both scans name `Parte Prima` only in the printed end-matter contents, while `Parte Seconda` also occurs in body pages; therefore `n-3` remains an intentionally unheaded logical container. | `s4_6_structural_contents_prereg.md`; issue #90 | S4.6-pre (#31), S4.6b (#34) | non-PLL positive/negative/conflict fixtures green; source locks exact; deterministic report reviewed before S4.6 evidence stamping | `BUILD` | `DONE` (2026-07-19) |
| S4.6d | **Evidence-bound visual structure review.** **Owners: engine contract + Book Review Workbench presentation.** The engine exposes a deterministic strict packet over the shared authoring inspection/status model, source-locks every visual, associates observations exact-only, formalizes the 22 legacy seeder warnings as non-authoritative lifecycle flags, and accepts only stale-safe single-container stamps through `engine.review_api`. The Workbench consumes that bridge without importing engine internals, keeps holds/history in its own ledger, and never writes the map/evidence artifacts directly. PLL copy-1 scan ranges are reproducibly derived for all 61 containers; copy 2 remains independently visible but intentionally unmapped until its pagination is registered. | `s4_6d_structure_review_contract.md`; issue #93; Workbench #2 | S4.6b (#34), S4.6c (#90) | generic packet/conflict fixtures + Workbench flows green; live PLL queue exposes 61 containers, 22 flags, associated observations, two source witnesses, and 61 copy-1 page ranges; full non-scale engine and Workbench suites green | `BUILD` | `WIP` (implementation green 2026-07-19; awaiting review/merge) |
| S4.6 | **Hand-author the PLL container map** (~61 containers). **Owner: Ben (human-in-the-loop).** Workflow specified — **and tooled (S4.6b, #34), with source observations reviewed first (S4.6c, #90) and the evidence-bound visual path supplied by S4.6d (#93):** draft seeded (`books/per_la_liberta/work/structure_map.json`, uncommitted — committing it is Ben's first authoring act) → inspect the advisory source report → review each current container and its registered scans in the Workbench → verify + single-node stamp → use `validate --watch`, `status`, `explain`, and `gate` as the independent CLI backstop → `gate` green = this row done. Observation ids may be cited in evidence prose only after the human checks their exact scan locators; no observation automatically becomes a heading binding or accepted evidence. Which file is edited; the known PLL skeleton (prefazione + 2 parts + 57 chapters + the embedded letter) **seeds candidates**; the **embedded-letter placement is verified at authoring** (pin the Mazzini-letter's chapter); review comments captured; minimum evidence beyond the skeleton recorded. S9 later makes the map *recognizer-suggested* — that comparison is **advisory** (gate = every recognizer disagreement surfaced + reviewed + accepted-or-explicitly-overridden, **human ruling authoritative** per S8.2), never "recognizer must match the hand map" | §3.5; D28/D29 (HITL premise) | S4.4, S4.6-pre (#31), S4.6a (#32), S4.6c (#90), S4.6d (#93) | the authored map validates against PLL's known skeleton; embedded-letter chapter pinned; review trail captured | `BUILD` (human) | `TODO` |
| S4.7 | **Scale check (D35).** Named ops under test — tree traversal, reference-integrity resolution, re-bind lookup — stay **sub-quadratic** across sizes (10⁴ → 10⁵ leaf nodes); budget is a rough **wall-clock + peak-memory ceiling that includes serialize + load + index-build** (a CLI pays the load cost every invocation), **machine ops only** (the human container-authoring ceiling is **O3**, not this gate). CI: a small always-on shape check; the full 10⁵ tier nightly/opt-in. **[Note 2026-07-17]** growth is asserted across **≥2 decades** (plan §5 as amended, B-6) — 10⁴→10⁵ is the top decade of the sweep, not the whole assertion. Reuses the S1.4/S1.5 store round-trip **primitives** (`save_stream`/`load_stream`) — not an existing fixture; the scale fixture itself is new S4.7 work (DR-7, Ben 2026-07-09). **Named input (delta re-audit 2026-07-02):** `evidence_findings` is O(entries × subtree) — measured 3.5s on a 3000-deep full-coverage chain; include the freeze×evidence composite gate in the 10⁵ measurements | §3.5; **D35** | S4.1, S4.2; issue #33 (**epic #85**, DoD item 4; INV-6/7 reds authored in #84) | scale tier: growth ratio sub-quadratic incl. load/index time; flat addressable storage holds at 10⁵ within budget | `BUILD` | `DONE` (2026-07-19; registered median-of-five PASS, #33/#85 closeout) |
| S4.7-E | **Deep-evidence composite traversal follow-up.** Resolved without changing the fixture or ceilings: `evidence_findings` now computes requested extents in one iterative bottom-up pass, reuses child subtree storage, and reuses a stored digest only after exact live-payload equality with its constructor-verified witness; changed payloads still hash through THE canonical producer, while malformed maps fall back to the scalar fail-loud path. The current source-locked unchanged D=3000 median-of-five is 0.143184s with a 1,417,936-byte peak against the fixed 2.0s / 512 MiB limits. The strict xfail was removed and both the shared pass and witness reuse are mutation-protected. | `s4_7_plan.md` DR-6; `docs/probes/s4_7_priority5_perf_baseline.json` | S4.7/#48 mechanism | D=3000 `evidence_findings` wall ≤2.0s and peak ≤512 MiB under the registered harness | `PERF` | `DONE` |
...
| ID | Deliverable | Refs | Deps | Done-when (tier) | Tag | St |
|---|---|---|---|---|---|---|
| S5.1 | `rebind_anchors` per node: **geometry region** (primary where matched, from D30 word-boxes), **fuzzy/fail-loud content fingerprint** (never the exact-substring the live tree tombstoned), **structural-path** tie-break. **Three explicit operating modes — `geometry-primary` \| `geometry-tie-break` \| `no-geometry` — selected by S2.0's outcome and recorded in the structure-map lineage / rebind config** (M2), so a re-bind result is interpretable after the fact. **[S2.2/#30, 2026-07-08] The re-gate ruled `geometry-tie-break` for PLL** — read it from `manifest.segmentation.geometry_mode` (the mode's lineage home until this rebind config exists; loader-surfaced as `seg.geometry_mode`, see `docs/probes/s2_2_regate.md`); on PLL geometry corroborates, content + structural-path lead. **S2.0 selected `geometry-primary` CONDITIONALLY — re-gate after S2.1** (post-audit, `docs/probes/s2_0_adversarial_audit.md`): primary-grade on confident two-column body pages (col-aware ordered mean 0.92 / 87% pass@0.85, n=30) but NOT settled (all-pages mean 0.851 / 73% pass; pre-registered naive metric 0.49); geometry primary where the S2.1 matcher is confident, fall back to content + structural-path where `geom.present=false`. Re-gate the mode at **S2.2** on the as-built S2.1 detector, on **mean + per-page pass-rate** (not a median). When a column-correct text witness exists it supplies reading order and geometry cross-checks; the geometric detector is the primary order source only on the no-witness branch. Re-attach algorithm with an **explicit confidence threshold** defaulting toward **fail-loud on doubt**: unique + above-threshold → bind; else fail loud. **Owns the authoring-evidence re-stamp protocol (post-audit remediation 2026-07-02, s4_plan §1.4.1b):** after a re-bind, `extent_digest`s are re-stamped **mechanically** exactly where the re-bind was unique + above threshold (the same bar as the bind itself); `decision_digest`s are **never machine-refreshed** (a decision drift is always a human re-verify); low-confidence re-binds route the affected entries to the human worklist instead of a silent re-stamp | §3.4, §3.6; D33, R2; `feedback_existing_path_failures_as_evidence` | S2, **S4.5** | re-binding tier: regenerated stream re-binds stored ids under unchanged geometry; the active mode is recorded in lineage | `BUILD` | `DONE` (#47, 2026-07-08; `structure/rebind.py` — RebindContext dual-hash baseline + joint monotone-tiling assignment **[shipped `O(K·N³)` — cubic `resolve_slot` (O(N²) windows × O(N) rescoring); the "near-linear" claim in `s5_1_plan.md` §1.3 is falsified-by-#33 and the docstring's `O(K·N²)` understates it; monotone-tiling mechanism superseded by S5.1-R's S4.7 diff-based re-anchoring (`DONE` 2026-07-19) — rewrite routed as S5.1 remediation G-1, issue #48; see `s4_7_plan.md` §7/DR-9]** + closed reason enum + bottom-up re-stamp; v2 schema born; hunt 22/22; wide+narrow audit caught & fixed a partial-rebind silent double-bind. S5.2 owns τ calibration + three-rate negatives + the persist form + regen-guard registration) |
| S5.1-R | **G-1 remediation (#48): replace the O(K·N³) re-bind matcher with diff-based annotation re-anchoring.** `resolve_slot`'s cubic window-scan×re-shingle cannot run at book scale (#33). Rev-3 design **(as amended ER-A1/ER-A2, 2026-07-17 — `s4_7_external_review_dispositions.md`)**: **anchored alignment** — unique k-gram anchors → LIS → capped per-gap `rapidfuzz.distance` opcodes (DR-1) → project each **slot** token-span through the composite projection (per-slot, B-1; DR-2) → post-alignment confidence gate = content ratio ≥ τ **and** boundary-anchor uniqueness in both streams **+ positional confirmation** (DR-3, the load-bearing surface); enrich stored anchors to prefix+exact+suffix (DR-4) → **schema v2→v3** (G-3, migrate fixtures + PLL probe, no dual-support); **preserve all S5.1 contracts** (DR-5: node_id, non-raising `rebind`+`assert_all_bound`, closed reason enum, re-stamp protocol, `RebindContext`, `RebindResult`/`Report` — Report preservation B-9-scoped: type/role preserved, fields amended in the v3 window); **zero-false-bind = strict-with-characterized-residual, S5.2 owns τ-zero** (DR-9; amended ER-A3: pre-S5.2 strict consumption requires a registered policy/calibration identity in the report); **contiguity = per-slot rebind-eligibility precondition** — a non-contiguous slot fails loud at rebind, the map stays valid (G-4 as amended 2026-07-17, ER-A4), multi-interval deferred. Per `s4_7_plan.md` RATIFIED rev 3 + the ER-A1…A5 amendments (2026-07-17) | `s4_7_plan.md` (RATIFIED rev 3) §2/§4/§8; DR-1…DR-9; G-1/G-3/G-4; build spec `s4_7_item2_invariants_plan.md` (**SIGNED OFF** Ben 2026-07-17) | S5.1 (#47, as-built); S4.7 (#33) | red-first INV-1…INV-7 + drift generator (**DR-3 gate locked only when INV-1 never-a-false-bind is seen red→green**); `bound ⊆ oracle` + anti-inertness proven; named ops sub-quadratic incl. serialize/load/index **and** an anchor-poor fixture on a correctness-at-density axis (§8.4); mutation all-killed + wide+narrow + Rule-A clean; #33 closed | `BUILD` | `DONE` (2026-07-19; #48; item-2 harness #84; scale gate #33; epic #85) |
| S5.1-RG | **Real-data confidence gate for the #48 mechanism [registered 2026-07-17 per `s4_7_item2_invariants_plan.md` rev 2.1 §1.4.2/§5.4]:** run the diff-based re-bind on a **true re-extraction of PLL** (post-S4.6 hand-authored container map) — the item-2 invariants prove the conditional property *under modeled drift* only, never real-re-extraction correctness, so this gate is **required, not optional**. Failures are **triaged before routing** (unmodeled-drift class / oracle-or-ground-truth defect / threshold-policy error / algorithmic #48 defect) — **builder classifies, Ben rules the routing [owner named 2026-07-17, A5]** — then routed per DR-9: mechanism defect → reopen #48; rate/calibration gap → S5.2. Also the registered home of the **copy2-derivation drift-distribution question** (removed from item 2; if ever revived it is a *descriptive comparison*, never fidelity validation) | `s4_7_plan.md` rev 3 §8 item 3 (DR-9); `s4_7_item2_invariants_plan.md` rev 2.1 §1.4 | S5.1-R (#48); **S4.6** | mechanism runs on the real re-extract; any mis-bind triaged + routed per the rule above; result recorded in `docs/probes/` | `GATE` | `TODO` |
| S5.2 | Re-binding negatives + **threshold calibration on a labeled truth set** + **regen-guard registration**. Calibration: a perturbation model (OCR-class char subs, re-segmentation, whitespace shifts, local reorder) over a stream with a **recorded identity map** (ground truth known by construction); report **false-bind / fail-loud / missed-bind separately**, treating **false-bind (silent-wrong) as the dominant cost → asymmetric threshold**; a **real re-extraction delta** sample is added *when available* (non-blocking). **[dep, 2026-07-08]** a *faithful* cleaned-substrate truth set needs native cleaned segmentation (**S3.1 / S9 / S8.2**); until those land, cleaned-substrate re-bind numbers carry **remap uncertainty** (the S4.7 cleaned rerun approximated the substrate via `is_noise_line` + a witness-id remap — see `docs/s4_7_cleaned_rerun_findings.md`; capped-desync collapsed ~12× / residual 0-of-1, but the yield figure is remap-contaminated). **Monotone-strictness property:** lowering signal strength (geometry-primary → tie-break → no-geometry) must **never lower the threshold or raise auto-bind coverage** — the correct response to "too many fail loud in no-geometry" is more human review, not a lower bar (`feedback_no_cheating_results`). Structure map joins the regeneration-guard family (one-way gate). **Agenda additions (post-audit 2026-07-02):** (a) register `atom_store.save_stream` in the regen-guard family — it currently overwrites a persisted stream silently, and post-freeze those bytes are the pinned authoring substrate; (b) define the transitional **two-canonical workspace state** for re-bind — `load_workspace_streams` hard-rejects a second canonical-kind stream, so decide whether re-bind gets a sanctioned side-by-side form (new-canonical-under-another-id) or a separate workspace | §3.4, §9; D33 | S5.1, **S8.1** | negative tier raises; the three rate classes measured; monotone-strictness property holds; regen-guard refuses silent regen of the map | `GATE` | `TODO` |
...
| ID | Deliverable | Refs | Deps | Done-when (tier) | Tag | St |
|---|---|---|---|---|---|---|
| S8.1 | Stale = **fail-loud** loader keyed on the lineage manifest; *why-stale → which-migration* routing. **Emits a typed stale report** (what changed · which class · what to run next) + a per-class **manual repair runbook**, each entry **asserted by a test, not just written**: changed raw hash → re-capture; changed normalizer → re-derive offsets; changed projection → re-attach L2/L3; changed dict/oracle → re-segment; **changed atom-store schema (S1.5) / changed relation-store schema (S7.1c) → migrate** — each produces a **distinct diagnostic + next command**. **Recovery defaults to dry-run/report;** any rewrite of map/atom/relation artifacts requires an explicit output target, a **snapshot-before-migrate** (mirroring `refine.py`), and a **new lineage entry** — the migration path itself **joins the regen-guard family** (unsafe recovery is worse than a block, Principle 4); **owns the S4.4 writer's crash-state recovery** (snapshot written, live map not superseded — permit an idempotent re-supersede when the existing snapshot's bytes equal the live map's; s4_plan §0.3 A-4) | §3.6; D14, D21 | S4.4, S1.5, S7.1c | negative: each stale class fails load until refresh/migrate **and** emits its own asserted diagnostic; recovery is dry-run-by-default + snapshot-guarded | `BUILD` | `TODO` |
| S8.2 | Decision provenance per entry (`human-approved\|plugin-suggested\|inherited`; re-suggestion never overwrites a human ruling); **C-corrections are reviewer-approved patches** to L2/L3, never auto-rewrite of L1; L1 supersession-by-new-stream + tombstone (kept-marked-invalid); **scope extends to L2 node identity: node retirement/tombstone semantics + id-reuse prohibition** (merging containers retires a `node_id` — s4_plan §3.B.7, M10) | §3.6; D14, D25 | S4.4, S5.1 | property: human ruling survives re-suggest; superseded atom ids stay addressable | `BUILD` | `TODO` |
