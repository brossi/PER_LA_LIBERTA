...
| S4.7-E | **Deep-evidence composite traversal follow-up.** Resolved without changing the fixture or ceilings: `evidence_findings` now computes requested extents in one iterative bottom-up pass, reuses child subtree storage, and reuses a stored digest only after exact live-payload equality with its constructor-verified witness; changed payloads still hash through THE canonical producer, while malformed maps fall back to the scalar fail-loud path. The source-locked unchanged D=3000 median-of-five is 0.148400s with a 1,285,736-byte peak against the fixed 2.0s / 512 MiB limits. The strict xfail was removed and both the shared pass and witness reuse are mutation-protected. | `s4_7_plan.md` DR-6; `docs/probes/s4_7_priority5_perf_baseline.json` | S4.7/#48 mechanism | D=3000 `evidence_findings` wall ≤2.0s and peak ≤512 MiB under the registered harness | `PERF` | `DONE` |
