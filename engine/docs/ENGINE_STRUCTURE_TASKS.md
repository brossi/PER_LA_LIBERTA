...
| S4.7-E | **Deep-evidence composite traversal follow-up.** The isolated `evidence_findings` D=3000 chain remains above the preregistered 2.0s wall ceiling after #48 while its Python-allocation peak is green. Preserve the strict residual, optimize the iterative evidence/subtree traversal, and rerun the unchanged preregistered fixture; any ceiling change requires a new preregistration, never an after-the-fact relaxation. This is the named DR-6 deferral and does not reopen the green anchored re-bind INV-6 result. | `s4_7_plan.md` DR-6; `docs/probes/s4_7_priority5_perf_baseline.json` | S4.7/#48 mechanism | D=3000 `evidence_findings` wall ≤2.0s and peak ≤512 MiB under the registered harness | `PERF` | `TODO` |
