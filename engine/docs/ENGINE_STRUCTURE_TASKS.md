...
| S5.1-R | **G-1 remediation (#48): replace the O(K·N³) re-bind matcher with diff-based annotation re-anchoring.** `resolve_slot`'s cubic window-scan×re-shingle cannot run at book scale (#33). Rev-3 design: `rapidfuzz.distance` opcodes (DR-1) → project each node token-span through the opcodes (DR-2) → post-alignment confidence gate = content ratio ≥ τ **and** boundary-anchor uniqueness in both streams (DR-3, the load-bearing surface); enrich stored anchors to prefix+exact+suffix (DR-4) → **schema v2→v3** (G-3, migrate fixtures + PLL probe, no dual-support); **preserve all S5.1 contracts** (DR-5: node_id, non-raising `rebind`+`assert_all_bound`, closed reason enum, re-stamp protocol, `RebindContext`, `RebindResult`/`Report`); **zero-false-bind = strict-with-characterized-residual, S5.2 owns τ-zero** (DR-9); **contiguity fail-loud precondition** — container subtree = one contiguous included span, multi-interval deferred (G-4). Per `s4_7_plan.md` RATIFIED rev 3 | `s4_7_plan.md` (RATIFIED rev 3) §2/§4/§8; DR-1…DR-9; G-1/G-3/G-4; build scaffold `s4_7_item2_invariants_plan.md` | S5.1 (#47, as-built); S4.7 (#33) | red-first INV-1…INV-7 + drift generator (**DR-3 gate locked only when INV-1 never-a-false-bind is seen red→green**); `bound ⊆ oracle` + anti-inertness proven; named ops sub-quadratic incl. serialize/load/index **and** an anchor-poor fixture on a correctness-at-density axis (§8.4); mutation all-killed + wide+narrow + Rule-A clean; #33 closed | `BUILD` | `TODO` (#48) |
