...
| S4.6-pre | **Freeze + persist the PLL atom streams** (row added 2026-07-02 from runway.md §3 item 1): canonical atom ids are **positional** (`canonical_NNNNN` from capture) — a capture-algorithm change renumbers the stream, and S4.6's hand-authored map references those ids *before* S5's re-bind machinery exists. Mitigation is procedural: capture the real copies through the live producers, persist via the S1.5 store into the PLL book workspace, record the stream hashes (the manifest's stream hashes turn later drift into S8.1-detectable staleness, not silent corruption). Cheap; **must precede S4.6** | runway.md §3.1 | S1.3a, S1.5 (#22); issue #31 | PLL witness + canonical streams persisted in the book workspace; hashes recorded; S4.6 authoring references only these frozen ids | `BUILD` | `TODO` |
