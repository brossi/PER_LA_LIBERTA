...
- **Complexity contract:** `O(N + K·B)` — boundary candidates per page limited to a band `B`
  (ruled 2026-07-03, P-2: 3× the max page bag size, ≈1.4K tokens; **superseded 2026-07-05: 16×,
  ≈20K tokens** — the 3× band clipped the DP against PLL's end-matter center drift; see the P-2
  row and `engine/docs/probes/s2_1_band_drift.md`), with *incremental* integer
  multiset scoring
...
|---|----------|----|-------------|--------|
| P-1 | copy3-blind page-locate calibration floor: ≥95% exact | DT-3 | #37 (calibration gate) | **RULED 2026-07-03: 95% for slice 1; SUPERSEDED by Ben 2026-07-05: two complementary clauses, BOTH must hold** — the single all-atom exact floor is unreachable by any point assignment (37% of copy3's single-page atoms are ≤5-token fragments that cannot discriminate adjacent pages), and each clause covers the other's structural blind spot. **Clause A**: exact ≥ 95% over single-page-truth atoms with **> 5 tokens** (the discriminable population; exactness catches the systematic one-page offset that a ±1 gate alone would pass at 100%). **Clause B**: within ±1 ≥ **98%** over **ALL** single-page-truth atoms (bounds worst-case error for the tiny atoms clause A cannot see — the band bug proved throws of +10 pages are a real class). Empty clause population = clause fails (no vacuous certification). Floors + the 5-token cutoff are CLI-tunable in the runner (`--calibration-exact-floor` / `--calibration-window-floor` / `--small-max-tokens`, ruled values as defaults) per Ben's tune-without-rework directive; measurement (`_calibrate_copy3_blind`) is separated from judgment (`_gate_verdict`, all branches exercised incl. both blind-spot scenarios). PLL slice-1 measures: A = 327/328 = 99.7%, B = 521/521 = 100%, ≤5-token slice 94/193 reported not gated. Evidence: `docs/probes/s2_1_band_drift.md` + `s2_1_run_stats.json`. |
| P-2 | DP band B = 3× max page bag (≈1.4K tokens) | DT-3 | #37 (page-locate) | **RULED 2026-07-03: accepted; SUPERSEDED by Ben 2026-07-05: B = 16× max page bag** — the first slice-1 run proved 3× narrower than the band-center prior's real error: centers (cumulative-bag-mass positions) drift up to +10 pages on PLL because end-matter pages carry bag mass but no stream tokens, so the clipped DP sent the calibration tail to 0% exact (53.6% overall) while the true pages held ~95% of the failing atoms' tokens. 16× (≈±16 pages on PLL) covers the worst observed drift with ~60% margin at ~10× locate cost (9s→88s, once per book); measured under the wide band the same calibration reads 99.6% within ±1, flat across the book. Full evidence, mechanism (the S-curve prior-error measurement), why meet-in-the-middle cannot help, and the re-centering escape hatch: `engine/docs/probes/s2_1_band_drift.md` + `books/per_la_liberta/probes/s2_1_band_drift_probe.py`. Guardrails: desert-fixture red (`test_locate_default_band_survives_end_matter_token_deserts`), edge-exact fixture reds any multiplier < 16, hunt rows `p2-band-regressed-to-superseded-3` / `p2-band-one-under-ruled`. |
...
Separately: the **hysteresis margin** (DT-7) and the column **`decision_threshold`** are ratified at
their consumer's run report — the **#39** column-detector run (`s2_1_column_probe.py` →
...
**RATIFIED by Ben 2026-07-06 at the #39 run report: `decision_threshold = 0.50`,
`hysteresis_margin = 0.15`** (threshold in the empty col2_score valley; margin covering the 5
transition pages). Now in `manifest.json` `segmentation.column_detector`; the loader builds the
`ColumnDetector` via `from_config`.
