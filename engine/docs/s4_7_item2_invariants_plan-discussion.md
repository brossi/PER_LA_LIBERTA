# S4.7 item-2 — the red-first invariant harness (build scaffold — DISCUSSION TRAIL)

> **This is the retained review trail, not the live spec.** It preserves verbatim the seven-pass
> adversarial review — pass 1 (`@@@@@@`/`======`) through pass 7 (`!!!!!!`/`!!!!!!`), escalating
> delimiter pairs per pass — plus the four parent-ruling blockquotes recorded at their escalation
> sites (`PR-1 RULED` / `PR-2 RULED` / `PR-4 RULED` / `PR-5 RULED`, Ben 2026-07-17). **The live
> build spec is [`s4_7_item2_invariants_plan.md`](s4_7_item2_invariants_plan.md) (rev 2, consolidated
> from this trail).** Edit that, not this. Where a later pass corrects an earlier response in this
> trail, the rev-2 spec carries only the final reconciled form.

---

# S4.7 item-2 — the red-first invariant harness (build scaffold)

**Status: DRAFT for review (2026-07-09…17) — superseded by `s4_7_item2_invariants_plan.md` rev 2.**
Scaffold for DoD item 2 of the RATIFIED `s4_7_plan.md` rev 3. Authored, reviewed inline over seven
passes (`@@@@@@`/`======` … `!!!!!!`/`!!!!!!`), rulings PR-1/2/4/5 recorded in situ 2026-07-17.
...
> **PR-5 RULED (Ben, 2026-07-17): Option A — the three premises are ratified #48 anchor-family
> constraints under DR-4** (dated amendment in `s4_7_plan.md` §3, "Parent rulings"), with **W scoped to
> the total footprint** (prefix + exact + suffix together; fixture plants the full `exact` content + >W
> context, twice). The DR-4 contract test joins item 2's build list; the minimal-anchor-fixture
> alternative is rejected. Ruled on the architecture-justifies-the-constraint bar: each premise is
> independently forced by the mechanism (re-derivability / drift-survival / locality + scale), not by the
> test's need for a red.
...
> **PR-1 RULED (Ben, 2026-07-17): the parent plan is formally reconciled** — `s4_7_plan.md` §4 INV-1 is
> amended to scope "non-unique anchor ⇒ `ambiguous`, never a lone bind" to **content evidence alone
> (`no-geometry` mode)**; geometry-mode disambiguation by an independently valid geometry signal is
> INV-5's per-mode territory (dated amendment + §3 "Parent rulings" entry). Item 2 runs INV-1 in
> `no-geometry` with the review-pinned quantifiers (whole-stream over the canonical included stream,
> both boundary anchors unique). The scaffold's INV-1 block may now finalize against the amended text.
...
> **PR-2 RULED (Ben, 2026-07-17): the parent plan is reconciled** — `s4_7_plan.md` §4 INV-3 is amended
> to the refined green condition (fail-loud OR correct-planted-destination + globally-valid structure
> map; never a bind elsewhere), with fail-loud-only documented as the current diff mechanism's expected
> outcome, not the ratified property (dated amendment + §3 "Parent rulings" entry). The scaffold's INV-3
> block may finalize against the amended text, with the separate cross-container move fixtures as pinned.
...
> **PR-4 RULED (Ben, 2026-07-17): the DoD is amended** (`s4_7_plan.md` §8 dated amendment + §3 "Parent
> rulings" entry) — INV-7 closes either **green** or **honestly UNRESOLVED** (DR-6
> characterize-and-defer): isolated single-purpose budget test with `xfail(strict, reason=<follow-up
> issue>)`, characterization artifact as the accepted deliverable, DoD item-3 state annotation.
> Ceiling pre-registered and independent of the measured baseline, never moved; deep-chain depth
> bounded with a pre-flight non-failure proof. The scaffold's INV-7 block may finalize against this
> two-end-state model.
