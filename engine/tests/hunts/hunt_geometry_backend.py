...
MUTANTS = [
    ...
    m("oob-guard-dropped",
      ...
      "            if False and (\n                x0 < rect.x0 - _RECT_TOLERANCE_PT",
      "test_isolated_off_page_box_is_dropped_and_counted_not_raised"),
    m("oob-right-clause-dropped",
      "                or x1 > rect.x1 + _RECT_TOLERANCE_PT\n"
      "                or y1 > rect.y1 + _RECT_TOLERANCE_PT",
      "                or y1 > rect.y1 + _RECT_TOLERANCE_PT",
      "test_isolated_off_page_box_is_dropped_and_counted_not_raised[right]"),
    # --- bounded drop-and-count (G-8 amended — ruled 2026-07-05) ---
    m("oob-bound-check-dropped",
      "        if oob > _OOB_PAGE_FRACTION_MAX * candidates:",
      "        if False:",
      "test_page_of_entirely_off_page_boxes_raises"),
    m("oob-bound-inclusive",
      "        if oob > _OOB_PAGE_FRACTION_MAX * candidates:",
      "        if oob >= _OOB_PAGE_FRACTION_MAX * candidates:",
      "test_off_page_fraction_exactly_at_bound_is_tolerated"),
    m("oob-bound-widened",
      "_OOB_PAGE_FRACTION_MAX = 0.20",
      "_OOB_PAGE_FRACTION_MAX = 2.0",
      "test_page_of_entirely_off_page_boxes_raises"),
    m("oob-bound-zeroed",
      "_OOB_PAGE_FRACTION_MAX = 0.20",
      "_OOB_PAGE_FRACTION_MAX = 0.0",
      "test_isolated_off_page_box_is_dropped_and_counted_not_raised"),
    m("oob-count-not-incremented",
      "                oob += 1",
      "                oob += 0",
      "test_isolated_off_page_box_is_dropped_and_counted_not_raised"),
    m("oob-conflated-into-dropped-counter",
      "                oob += 1",
      "                dropped += 1",
      "test_isolated_off_page_box_is_dropped_and_counted_not_raised"),
    m("oob-box-emitted-anyway",
      ...
      "                    oob_example = (x0, y0, x1, y1)",
      "test_isolated_off_page_box_is_dropped_and_counted_not_raised"),
    m("oob-denominator-raw-boxes",
      "        if oob > _OOB_PAGE_FRACTION_MAX * candidates:",
      "        if oob > _OOB_PAGE_FRACTION_MAX * (candidates + dropped):",
      "test_oob_fraction_denominator_is_candidates_not_raw_boxes"),
    m("oob-counter-banked-after-raise",
      "        self.dropped_boxes[num] = dropped\n        self.oob_boxes[num] = oob",
      "        self.dropped_boxes[num] = dropped",
      "test_off_page_fraction_above_bound_raises_and_banks_the_counter"),
    # --- delta-audit additions (2026-07-05): mutants the first oob battery left alive ---
    m("finite-guard-narrowed-to-nan",
      "            if not all(math.isfinite(c) for c in (x0, y0, x1, y1)):",
      "            if any(math.isnan(c) for c in (x0, y0, x1, y1)):",
      "test_non_finite_box_coordinate_raises_geometry_error_not_valueerror"),
    m("oob-keying-hardcoded",
      "        self.oob_boxes[num] = oob",
      "        self.oob_boxes[1] = oob",
      "test_multi_page_range_reads_the_requested_pages_with_their_own_numbers_and_rects"),
    m("oob-key-skipped-when-zero",
      "        self.oob_boxes[num] = oob",
      "        if oob:\n            self.oob_boxes[num] = oob",
      "test_blank_page_yields_zero_words_successfully"),
    m("oob-bound-drifted-up",
      "_OOB_PAGE_FRACTION_MAX = 0.20",
      "_OOB_PAGE_FRACTION_MAX = 0.24",
      "test_off_page_fraction_just_above_bound_raises"),
