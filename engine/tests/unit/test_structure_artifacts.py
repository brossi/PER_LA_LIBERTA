...
PUBLIC_SURFACE_BY_CONCERN = {
    ...
    "geom_match": ("normalize_tokens", "locate_pages", "match_stream", "MatchOutcome",
                   "build_geom_sidecar", "attach_geometry", "AttachResult", "AttachOutcome"),
    "geom_sidecar": ("GeomSidecar", "SourceScan", "PageRecord", "AtomRecord", "AtomPages",
                     "geom_sidecar_path", "save_geom_sidecar", "load_geom_sidecar",
                     "assert_source_scan_matches", "assert_auto_absent_tripwire"),
    "segmentation": ("SEGMENTATION_VERSION", "DensityBand", "DensityClassifier", "DensityVerdict",
                     "PageDensityFeatures", "edge_strip", "ink_fraction_from_pixmap",
                     "is_alpha_token", "page_density_features",
                     # S2.1.5 (#39) — column / reading-order detector + cross-page prior
                     "COLUMN_DETECTOR_VERSION", "ColumnDetector", "ColumnEvidence", "ColumnVerdict",
                     "PageColumnInput", "PageColumnVerdict", "detect_columns", "ordered_coverage",
                     "reading_order", "resolve_reading_columns"),
...
EXPECTED_PUBLIC_SURFACE = frozenset(
    {
        ...
        # S2.1.3 — matcher + page-locate + attach overlay; geometry sidecar (#37)
        "normalize_tokens",
        "locate_pages",
        "match_stream",
        "MatchOutcome",
        ...
        # S2.1.5 — column / reading-order detector + cross-page prior (segmentation)
        ...
        "workspace_reader",
        # S4.6a post-audit remediation: typed gate error + findings producer + engine writer
        "EvidenceGateError",
